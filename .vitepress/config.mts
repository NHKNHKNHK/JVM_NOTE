...
const base = process.env.GITHUB_ACTIONS === 'true' ? '/JVM NOTE/' : '/'
...
export default withDrawio(defineConfig({
  ...
  themeConfig: {
    ...
    nav: [
      ...
      {
        text: '指南', items: [
          { text: '内存与垃圾回收', link: '/JVM上篇：内存与垃圾回收篇/01-JVM与Java体系结构/README' },
          { text: '字节码与类的加载', link: '/JVM中篇：字节码与类的加载篇/01-class文件结构/README' },
          { text: '性能监控与调优', link: '/JVM下篇：性能监控与调优/01-概述篇/README' }
        ]
      },
    ...
    sidebar: [
      {
        text: '内存与垃圾回收篇',
        items: [
          { text: 'JVM与Java体系结构', link: '/JVM上篇：内存与垃圾回收篇/01-JVM与Java体系结构/README' },
          { text: '类加载子系统', link: '/JVM上篇：内存与垃圾回收篇/02-类加载子系统/README' },
          { text: '运行时数据区及程序计数器', link: '/JVM上篇：内存与垃圾回收篇/03-运行时数据区及程序计数器/README' },
          { text: '虚拟机栈', link: '/JVM上篇：内存与垃圾回收篇/04-虚拟机栈/README' },
          { text: '本地方法接口和本地方法栈', link: '/JVM上篇：内存与垃圾回收篇/05-本地方法接口和本地方法栈/README' },
          { text: '堆', link: '/JVM上篇：内存与垃圾回收篇/06-堆/README' },
          { text: '方法区', link: '/JVM上篇：内存与垃圾回收篇/07-方法区/README' },
          { text: '对象实例化及直接内存', link: '/JVM上篇：内存与垃圾回收篇/08-对象实例化及直接内存/README' },
          { text: '执行引擎', link: '/JVM上篇：内存与垃圾回收篇/09-执行引擎/README' },
          { text: 'StringTable', link: '/JVM上篇：内存与垃圾回收篇/10-StringTable/README' },
          { text: '垃圾回收概述及算法', link: '/JVM上篇：内存与垃圾回收篇/11-垃圾回收概述及算法/README' },
          { text: '垃圾回收相关概念', link: '/JVM上篇：内存与垃圾回收篇/12-垃圾回收相关概念/README' },
          { text: '垃圾回收器', link: '/JVM上篇：内存与垃圾回收篇/13-垃圾回收器/README' },
        ]
      },
      {
        text: '字节码与类加载篇',
        items: [
          { text: 'class文件结构', link: '/JVM中篇：字节码与类加载篇/01-class文件结构/README' },
          { text: '字节码指令集', link: '/JVM中篇：字节码与类的加载篇/02-字节码指令集/README' },
          { text: '类加载过程', link: '/JVM中篇：字节码与类加载篇/03-类的加载过程（类的生命周期）详解/README' },
          { text: '再谈类的加载器', link: '/JVM中篇：字节码与类加载篇/04-再谈类的加载器/README' },
        ]
      },
      {
        text: '性能监控与调优篇',
        items: [
          { text: '概述篇', link: '/JVM下篇：性能监控与调优篇/01-概述篇/README' },
          { text: 'JVM监控及诊断工具-命令行篇', link: '/JVM下篇：性能监控与调优篇/02-JVM监控及诊断工具-命令行篇/README' },
          { text: 'JVM监控及诊断工具-GUI篇', link: '/JVM下篇：性能监控与调优篇/03-JVM监控及诊断工具-GUI篇/README' },
          { text: 'JVM运行时参数', link: '/JVM下篇：性能监控与调优篇/04-JVM运行时参数/README' },
          { text: '分析GC日志', link: '/JVM下篇：性能监控与调优篇/05-分析GC日志/README' },
          { text: '补充：浅堆深堆与内存泄露', link: '/JVM下篇：性能监控与调优篇/补充：浅堆深堆与内存泄露/README' },
          { text: '补充：使用OQL语言查询对象信息', link: '/JVM下篇：性能监控与调优篇/补充：使用OQL语言查询对象信息/README' },
        ]
      }
    ],
